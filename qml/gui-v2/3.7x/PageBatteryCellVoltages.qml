...
Page {
    ...
    function getCellTextColor(cell) {
        ...
        if (!voltageCellItem.valid) {
            return Theme.color_font_primary;
        }
        if (balanceCellItem.valid && balanceCellItem.value == "1") {
            return "#BD7624";
        }
        if (cellMax.valid && cellMax.value == voltageCellItem.value) {
            return "#BF4845";
        }
        if (cellMin.valid && cellMin.value == voltageCellItem.value) {
            return "#295C91";
        }
        return Theme.color_font_primary;
    }
